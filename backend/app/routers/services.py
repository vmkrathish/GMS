...
log = logging.getLogger("gms.services")
...
def get_services(
    ...
    rows = db.execute(text(f"""
        ...
        -- Ranking uses a NEUTRAL placeholder (platform-wide average
        -- rating) for providers with zero reviews yet, instead of
        -- ranking them as if they had a 0 rating — that was the
        -- actual bug behind "new providers don't appear in search":
        -- COALESCE(..., 0) buried every brand-new provider at the
        -- absolute bottom of every paginated result, permanently,
        -- since 0 loses to literally any real rating. This is
        -- SORT-ONLY — the customer-facing average_rating column
        -- above still honestly shows 0 / "no reviews yet", never a
        -- fake number.
        ORDER BY COALESCE(r.avg_rating, (SELECT COALESCE(AVG(rating), 4.0) FROM reviews)) DESC,
                 s.created_at DESC
...
def get_recommended_services(
    ...
    if home_lat is None or home_lng is None:
        rows = db.execute(text(f"""
            ...
            LEFT JOIN ({RATING_SQL}) r ON r.provider_id = s.provider_id
            WHERE {' AND '.join(conditions)}
            -- Same fix as /api/services — see the detailed comment
            -- there. Sort-only neutral placeholder for zero-review
            -- providers, never shown to the customer as their real
            -- rating.
            ORDER BY COALESCE(r.avg_rating, (SELECT COALESCE(AVG(rating), 4.0) FROM reviews)) DESC,
                     s.created_at DESC
            LIMIT :limit
    ...
    for row in rows:
        ...
        # Same fix as the SQL queries above (see get_services /
        # get_recommended_services's no-location fallback): a
        # brand-new provider with zero reviews yet should not score
        # as if they had the WORST possible rating (0/5) — that's
        # what was quietly burying new providers in "Recommended for
        # You" too, since rating carries real weight in this
        # composite score. Neutral midpoint (0.5) for zero reviews;
        # anyone with real reviews still scores on their actual
        # average as before — this never fabricates a displayed
        # rating, only affects internal ranking.
        review_count = int(d.get("review_count") or 0)
        rating_score = (float(d["average_rating"]) / 5) if review_count > 0 else 0.5
...
def create_service(body: dict, user: User = Depends(get_current_user),
    ...
    if not title or not city or not category_id:
        return _fail(400, "title, category_id and city are required.")
    ...
    # The provider does NOT set the initial price — GMS calculates
    # it from market reference data (general + locality, blended).
    # Any "price" the request body includes is deliberately ignored
    # here; this is the platform-controlled base price the spec
    # asked for, not a provider-entered number.
    calculated_base = pricing.calculate_base_price(
        db, int(category_id), price_unit, city)
    if calculated_base is None:
        # No market reference data exists yet for this exact
        # category+model+locality combination — honest fallback
        # rather than blocking signup entirely. An admin adding
        # reference data later does not retroactively change this;
        # see pricing.py's get_or_create_pricing_state.
        calculated_base = Decimal("500.00")
        log.info("create_service: no market reference for category_id=%s "
                 "price_unit=%s city=%s — using platform default base",
    ...
    db.add(svc)
    db.flush()
    pricing.get_or_create_pricing_state(db, svc)

    # A new signup always starts as role='customer' — this is the
    # actual moment they become a provider: the instant they
    # successfully add a service. Without this, a brand-new user
    # could create services that saved correctly to the database but
    # never behaved like a provider anywhere else in the app, since
    # nothing ever flips their role away from 'customer'.
    if user.role == "customer":
        user.role = "provider"
        log.info("create_service: promoted user_id=%s from customer to "
                 "provider (first service created)", user.id)
...
def update_service(service_id: int, body: dict,
    ...
    if "price" in body:
        # Backend enforcement — spec item 9/10: never trust frontend
        # validation alone. Rejects anything outside the currently
        # allowed range (base price, or up to the eligible ceiling),
        # with the exact message format the spec asked for.
        requested = Decimal(str(body["price"]))
        is_valid, message = pricing.validate_requested_price(db, svc, requested)
        if not is_valid:
            db.rollback()
            return _fail(400, message)
        svc.price = float(requested)
